...
class UsersController extends Controller
{
    ...
    public function updatePost(Request $request,$id){
		...
		request('img')->move("upload/users",$file_name);
		DB::table("users")->where("id","=",$id)->update(["img"=>$file_name]);
    ...
    public function createPost(Request $request){
        ...
        $email = request("email");
		$file_name = request("img")->getClientOriginalName();
		$img = time()."_".$file_name;
		request('img')->move("upload/users",$img);
        $password = $request->get("password");
        $password = Hash::make($password);
        //update name
        DB::table("users")->insert(["name"=>$name,"email"=>$email,"img"=>$img,"password"=>$password]);
        return redirect(url('backend/users'));
    }

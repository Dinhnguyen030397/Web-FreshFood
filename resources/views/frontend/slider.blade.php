<div class="menu_slider">
        <div class="sub_menu">
            <ul>
                <h1>DANH MỤC</h1>
                <li>
                    <a href=""> <i class="fa-solid fa-circle-arrow-right"></i> Rau sạch</a>
                    <ul>
                        <li >
                            <a href="">Menu demo 1</a>
                            <ul>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                            </ul>
                        </li>
                        <li >
                            <a href="">Menu demo 1</a>
                            <ul>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                            </ul>
                        </li>

                        <li >
                            <a href="">Menu demo 1</a>
                            <ul>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                            </ul>
                        </li>

                        <li >
                            <a href="">Menu demo 1</a>
                            <ul>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                            </ul>
                        </li>

                        <li >
                            <a href="">Menu demo 1</a>
                            <ul>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                            </ul>
                        </li>
                    </ul>
                </li>

                <li>
                    <a href=""> <i class="fa-solid fa-circle-arrow-right"></i> Thịt tươi</a>
                    <ul>
                        <li >
                            <a href="">Menu demo 1</a>
                            <ul>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                            </ul>
                        </li>
                        <li >
                            <a href="">Menu demo 1</a>
                            <ul>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                            </ul>
                        </li>

                        <li >
                            <a href="">Menu demo 1</a>
                            <ul>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                            </ul>
                        </li>

                        <li >
                            <a href="">Menu demo 1</a>
                            <ul>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                            </ul>
                        </li>

                        <li >
                            <a href="">Menu demo 1</a>
                            <ul>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                            </ul>
                        </li>
                    </ul>
                
                </li>

                <li>
                    <a href=""> <i class="fa-solid fa-circle-arrow-right"></i> Hải sản</a>
                    <ul>
                        <li >
                            <a href="">Menu demo 1</a>
                            <ul>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                            </ul>
                        </li>
                        <li >
                            <a href="">Menu demo 1</a>
                            <ul>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                            </ul>
                        </li>

                        <li >
                            <a href="">Menu demo 1</a>
                            <ul>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                            </ul>
                        </li>

                        <li >
                            <a href="">Menu demo 1</a>
                            <ul>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                            </ul>
                        </li>

                        <li >
                            <a href="">Menu demo 1</a>
                            <ul>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                            </ul>
                        </li>
                    </ul>
                </li>

                <li>
                    <a href=""> <i class="fa-solid fa-circle-arrow-right"></i> Hoa quả tươi</a>
                    <ul>
                        <li >
                            <a href="">Menu demo 1</a>
                            <ul>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                            </ul>
                        </li>
                        <li >
                            <a href="">Menu demo 1</a>
                            <ul>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                            </ul>
                        </li>

                        <li >
                            <a href="">Menu demo 1</a>
                            <ul>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                            </ul>
                        </li>

                        <li >
                            <a href="">Menu demo 1</a>
                            <ul>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                            </ul>
                        </li>

                        <li >
                            <a href="">Menu demo 1</a>
                            <ul>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                            </ul>
                        </li>
                    </ul>
                </li>

                <li>
                    <a href=""> <i class="fa-solid fa-circle-arrow-right"></i> Nhập khẩu</a>
                    <ul>
                        <li >
                            <a href="">Menu demo 1</a>
                            <ul>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                            </ul>
                        </li>
                        <li >
                            <a href="">Menu demo 1</a>
                            <ul>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                            </ul>
                        </li>

                        <li >
                            <a href="">Menu demo 1</a>
                            <ul>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                            </ul>
                        </li>

                        <li >
                            <a href="">Menu demo 1</a>
                            <ul>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                            </ul>
                        </li>

                        <li >
                            <a href="">Menu demo 1</a>
                            <ul>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                            </ul>
                        </li>
                    </ul>
                </li>

                <li>
                    <a href=""> <i class="fa-solid fa-circle-arrow-right"></i> Cá Nhật Bản</a>
                    <ul>
                        <li >
                            <a href="">Menu demo 1</a>
                            <ul>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                            </ul>
                        </li>
                        <li >
                            <a href="">Menu demo 1</a>
                            <ul>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                            </ul>
                        </li>

                        <li >
                            <a href="">Menu demo 1</a>
                            <ul>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                            </ul>
                        </li>

                        <li >
                            <a href="">Menu demo 1</a>
                            <ul>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                            </ul>
                        </li>

                        <li >
                            <a href="">Menu demo 1</a>
                            <ul>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                            </ul>
                        </li>
                    </ul>
                </li>

                <li>
                    <a href=""> <i class="fa-solid fa-circle-arrow-right"></i> Rau Đà Lạt</a>
                    <ul>
                        <li >
                            <a href="">Menu demo 1</a>
                            <ul>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                            </ul>
                        </li>
                        <li >
                            <a href="">Menu demo 1</a>
                            <ul>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                            </ul>
                        </li>

                        <li >
                            <a href="">Menu demo 1</a>
                            <ul>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                            </ul>
                        </li>

                        <li >
                            <a href="">Menu demo 1</a>
                            <ul>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                            </ul>
                        </li>

                        <li >
                            <a href="">Menu demo 1</a>
                            <ul>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                            </ul>
                        </li>
                    </ul>
                </li>

                <li>
                    <a href=""> <i class="fa-solid fa-circle-arrow-right"></i> Hoa quả nhập</a>
                    <ul>
                        <li >
                            <a href="">Menu demo 1</a>
                            <ul>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                            </ul>
                        </li>
                        <li >
                            <a href="">Menu demo 1</a>
                            <ul>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                            </ul>
                        </li>

                        <li >
                            <a href="">Menu demo 1</a>
                            <ul>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                            </ul>
                        </li>

                        <li >
                            <a href="">Menu demo 1</a>
                            <ul>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                            </ul>
                        </li>

                        <li >
                            <a href="">Menu demo 1</a>
                            <ul>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                            </ul>
                        </li>
                    </ul>
                </li>
               
                <li>
                    <a href=""> <i class="fa-solid fa-circle-arrow-right"></i> Xem thêm</a>
                    <ul>
                        <li >
                            <a href="">Menu demo 1</a>
                            <ul>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                            </ul>
                        </li>
                        <li >
                            <a href="">Menu demo 1</a>
                            <ul>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                            </ul>
                        </li>

                        <li >
                            <a href="">Menu demo 1</a>
                            <ul>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                            </ul>
                        </li>

                        <li >
                            <a href="">Menu demo 1</a>
                            <ul>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                            </ul>
                        </li>

                        <li >
                            <a href="">Menu demo 1</a>
                            <ul>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                                <li><a href="">Menu demo 2</a></li>
                            </ul>
                        </li>
                    </ul>
                </li>
            </ul>
        </div>
        <div class="container_slider">
            <div class="slider"> <img src="{{ asset('upload/slider_1.png') }}" alt=""></div>
            <div class="slider"> <img src="{{ asset('upload/slider_1.png') }}" alt=""></div>
            <div class="slider"> <img src="{{ asset('upload/slider_1.png') }}" alt=""></div>
        </div>
    </div> 
    <div class="banner">
        <img src="{{ asset('upload/banner1.png') }}" alt="">
        <img src="{{ asset('upload/banner2.png') }}" alt="">
        <img src="{{ asset('upload/banner3.png') }}" alt="">
    </div>